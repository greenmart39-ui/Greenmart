...
function renderProds(){
  ...
  if(catFilter!=='all'){
    // single category — no grouping header needed
    grid.innerHTML=prods.map(p=>renderProdCard(p)).join('');
  } else {
    // Group by category
    const cats=[...new Set(prods.map(p=>p.category))];
    const icons={'Grains & Rice':'🌾','Dairy':'🥛','Beverages':'🥤','Snacks':'🍿','Vegetables':'🍅','Fruits':'🍌','Meat & Fish':'🍗','Bakery':'🍞','Household':'🧴','Personal Care':'🧼'};
    grid.innerHTML=cats.map(cat=>{
      ...
      ${catProds.map(p=>renderProdCard(p)).join('')}`;
    }).join('');
  }
}
...
function renderProdCard(p){
  ...
  const otherPrice=currentUser.type==='retail'?p.wholesalePrice:p.retailPrice;
  ...
  const btbNudge=(currentUser.type==='wholesale'&&selPayment!=='cash'&&cOffer&&cOffer.showSavings)?
    `<div style="font-size:13px;color:var(--green);background:rgba(46,213,115,.08);border:1px solid rgba(46,213,115,.15);border-radius:5px;padding:4px 8px;margin-top:4px">💵 Cash ගෙවූ විට ඒකකයකට <strong>රු. ${(cOffer.discountAmt||cOffer.discount||0).toFixed(0)}</strong> ඉතිරිය!</div>`:'';
  // Show both prices for retail customers — highlight "අපේ මිල" (costPrice)
  // Show both prices for wholesale customers — highlight "අපේ මිල" (wholesalePrice, billing price)
  let dualPrice;
  if(currentUser.type==='retail'){
    ...
    <div style="display:flex;gap:8px;align-items:stretch;flex-wrap:wrap;margin-top:4px">
      <div>
        <div style="font-size:13px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">විකිණීමේ මිල</div>
      ...
      <div style="background:rgba(232,255,71,.12);border:2px solid rgba(232,255,71,.4);border-radius:6px;padding:4px 10px;display:flex;flex-direction:column;justify-content:center">
        <div style="font-size:13px;color:var(--accent);text-transform:uppercase;letter-spacing:1px;margin-bottom:1px">⭐ අපේ මිල</div>
        <span style="font-family:'Syne',sans-serif;font-size:13px;font-weight:700;color:var(--accent)">රු. ${billingPrice.toFixed(2)}</span>
      </div>
    </div>`;
  } else {
    // Wholesale: show retailPrice (wikunum mila) crossed out, wholesalePrice (ape mila) highlighted
    dualPrice=`
    <div style="display:flex;gap:8px;align-items:stretch;flex-wrap:wrap;margin-top:4px">
      <div>
        <div style="font-size:13px;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">විකිණීමේ මිල</div>
        <span style="font-size:13px;color:var(--text3);text-decoration:line-through">රු. ${p.retailPrice.toFixed(2)}</span>
      </div>
      <div style="background:rgba(232,255,71,.12);border:2px solid rgba(232,255,71,.4);border-radius:6px;padding:4px 10px;display:flex;flex-direction:column;justify-content:center">
        <div style="font-size:13px;color:var(--accent);text-transform:uppercase;letter-spacing:1px;margin-bottom:1px">⭐ අපේ මිල</div>
  ...
  return `<div class="pc">
    ...
    <div class="pc-body">
      ...
      ${dualPrice}
      ${bOffer&&bAmt>0?`<div style="font-size:13px;color:var(--teal);margin-top:3px">📊 ${bOffer.minQty}+ ගත් විට ඒකකයකට <strong>රු. ${bAmt.toFixed(0)} off!</strong></div>`:''}
      ${btbNudge}
    </div>
